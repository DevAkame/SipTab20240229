...
export default function StocksSubCategoryUpdateForm(props) {
  const {
    id: idProp,
    stocksSubCategory: stocksSubCategoryModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = stocksSubCategoryRecord
      ? {
          ...initialValues,
          ...stocksSubCategoryRecord,
          stkscategoryID,
          StockDataPools: linkedStockDataPools,
        }
      : initialValues;
    setName(cleanValues.name);
    setStkscategoryID(cleanValues.stkscategoryID);
  ...
  const [stocksSubCategoryRecord, setStocksSubCategoryRecord] = React.useState(
    stocksSubCategoryModelProp
  );
  ...
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(StocksSubCategory, idProp)
        : stocksSubCategoryModelProp;
      setStocksSubCategoryRecord(record);
      const stkscategoryIDRecord = record
        ? await record.stkscategoryID
        : undefined;
      setStkscategoryID(stkscategoryIDRecord);
    ...
    queryData();
  }, [idProp, stocksSubCategoryModelProp]);
  ...
  return (
    <Grid
        ...
        try {
          ...
          stockDataPoolsToUnLink.forEach((original) => {
            if (!canUnlinkStockDataPools) {
              throw Error(
                `StockDataPool ${original.id} cannot be unlinked from StocksSubCategory because stockssubcategoryID is a required field.`
              );
            }
            promises.push(
              DataStore.save(
                StockDataPool.copyOf(original, (updated) => {
                  updated.stockssubcategoryID = null;
                })
              )
            );
          });
          stockDataPoolsToLink.forEach((original) => {
            promises.push(
              DataStore.save(
                StockDataPool.copyOf(original, (updated) => {
                  updated.stockssubcategoryID = stocksSubCategoryRecord.id;
                })
              )
            );
          });
          ...
          promises.push(
            DataStore.save(
              StocksSubCategory.copyOf(stocksSubCategoryRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "StocksSubCategoryUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || stocksSubCategoryModelProp)}
